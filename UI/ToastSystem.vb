...
Namespace Skye.UI
    ...
    Public Class ToastOptions
        ...
        ' Basic Properties
        ''' <summary>
        ''' Sets the title text of the toast notification. This is typically displayed in bold at the top of the toast window.
        ''' </summary>
        Public Property Title As String
        ''' <summary>
        ''' Sets the message text of the toast notification. This is displayed below the title.
        ''' </summary>
        ...
        ' Optional Properties
        ''' <summary>
        ''' If set to True, the toast will play a system sound when displayed. Default is False.
        ''' </summary>
        Public Property PlaySound As Boolean = False
        ''' <summary>
        ''' Sets the duration (in milliseconds) for which the toast notification will be displayed before automatically closing. Default is 3000 ms (3 seconds).
        ''' </summary>
        ...
        ' Display Properties
        ''' <summary>
        ''' Sets the width of the toast notification window in pixels. Default is 100 pixels.
        ''' </summary>
        Public Property Width As Integer = 100
        ''' <summary>
        ''' Sets the height of the toast notification window in pixels. Default is 80 pixels.
        ''' </summary>
        Public Property Height As Integer = 80
        ''' <summary>
        ''' Sets the margin (in pixels) between the toast notification and the edges of the screen. This affects the positioning of the toast based on the selected ToastLocation. Default is 10 pixels.
        ''' </summary>
        Public Property Margin As Integer = 10
        ''' <summary>
        ''' Sets the image to be displayed in the toast notification. This can be used to show a custom image alongside the title and message. If both Image and Icon are set, Image takes precedence. Default is Nothing (no image).
        ''' </summary>
        Public Property Image As Image = Nothing
        ''' <summary>
        ''' Sets the icon to be displayed in the toast notification. This is typically a small icon that represents the type of notification. If both Image and Icon are set, Image takes precedence. Default is Nothing (no icon).
        ''' </summary>
        Public Property Icon As Icon = Nothing
        ''' <summary>
        ''' Sets the font for the title text of the toast notification. Default is Segoe UI, 10pt, Bold.
        ''' </summary>
        Public Property TitleFont As Font = New Font("Segoe UI", 10, FontStyle.Bold)
        ''' <summary>
        ''' Sets the font for the message text of the toast notification. Default is Segoe UI, 10pt, Regular.
        ''' </summary>
        Public Property MessageFont As Font = New Font("Segoe UI", 10, FontStyle.Regular)
        ''' <summary>
        ''' Sets the background color of the toast notification window. Default is a dark gray (RGB: 40, 40, 40).
        ''' </summary>
        Public Property BackColor As Color = Color.FromArgb(40, 40, 40)
        ''' <summary>
        ''' Sets the border color of the toast notification window. Default is White.
        ''' </summary>
        Public Property BorderColor As Color = Color.White
        ''' <summary>
        ''' Sets the text color (foreground color) of the title and message in the toast notification. Default is White.
        ''' </summary>
        Public Property ForeColor As Color = Color.White
        ''' <summary>
        ''' Sets the predefined location on the screen where the toast notification will appear, such as TopLeft, BottomRight, etc. Default is BottomRight.
        ''' </summary>
        Public Property Location As ToastLocation = ToastLocation.BottomRight
        ''' <summary>
        ''' Sets the width of the border around the toast notification window in pixels. Default is 2 pixels.
        ''' </summary>
        ...
        ''' <summary>
        ''' If set to True, the toast notification will display a drop shadow effect. Default is True.
        ''' </summary>
        Public Property Shadow As Boolean = True

        ' Click Action
        ''' <summary>
        ''' Sets the action to be executed when the toast notification is clicked. This can be used to perform a specific task or open a related window when the user interacts with the toast. If no action is set, clicking the toast will simply close it.
        ''' </summary>
    ...
    Friend Class ToastManager
        ...
        Public Shared Sub Show(opts As ToastOptions)
            ...
            ' Local variable to hold the handler reference for self-unsubscribing
            Dim handler As ToastWindow.ToastClosedEventHandler = Nothing
            handler = Sub()
                          RemoveHandler toast.ToastClosed, handler
                          If _activeToast Is toast Then
                              _activeToast = Nothing
                          End If
                      End Sub
            AddHandler toast.ToastClosed, handler

        End Sub
        Public Shared Sub Hide()
            If _activeToast IsNot Nothing Then
                Dim currentToast = _activeToast
                _activeToast = Nothing ' Clear immediately to release the reference
                currentToast.CloseToastWindow()
            End If
        End Sub
    ...
    Friend Class LayeredToastWindow
        ...
        Private Sub RenderToast(g As Graphics)
            ...
            Dim rightInset As Integer = padding + _opts.BorderWidth
            Dim wAvail As Integer = w - textX - rightInset
        ...
        Private Sub AutoWidthToast()
            Dim padding As Integer = TOAST_PADDING
            Dim borderInset As Integer = _opts.BorderWidth
            ...
            ' Left margin before text starts
            Dim textX As Integer = padding + borderInset
            If iconSpace > 0 Then
                textX += iconSpace
            End If
                ...
                Using g As Graphics = Graphics.FromImage(bmp)
                    ...
                    Using sf As StringFormat = CType(StringFormat.GenericTypographic.Clone(), StringFormat)
                        ...
                        If Not String.IsNullOrEmpty(_opts.Message) Then
                            messageWidth = g.MeasureString(_opts.Message, _opts.MessageFont, PointF.Empty, sf).Width
                        End If
                    End Using
                End Using
            End Using
    ...
    Friend Class ToastWindow
        ...
        Private Function ComputeLocationPosition(loc As ToastLocation, toastWidth As Integer, toastHeight As Integer, anchor As Point) As Point
            ...
            Dim x As Integer
            Dim y As Integer
            Dim margin As Integer = _opts.Margin
